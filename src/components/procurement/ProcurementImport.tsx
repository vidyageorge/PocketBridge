...
export function ProcurementImport() {
  const { replaceFromFile } = useProcurement();
  ...
  const processFile = useCallback(
    async (file: File) => {
      ...
      if (!extension || !['xlsx', 'xls'].includes(extension)) {
        setError('Please upload the procurement Excel file (.xlsx or .xls).');
      ...
      try {
        ...
        if (count === 0) {
          setError('No procurement rows found. Check that the file has monthly sheets (May-25, June-25, etc.).');
          setIsProcessing(false);
          return;
        }
        setImportedCount(count);
      } catch {
        setError('Unable to read this file. Use the procurement Excel workbook format.');
      } finally {
        setIsProcessing(false);
      }
    },
    ...
    <div className="pb-content-card rounded-xl p-4">
      ...
      {importedCount !== null && (
        <p className="mt-2 flex items-center gap-2 text-sm text-primary">
          ...
          {importedCount} records loaded across all monthly sheets.
        </p>
      )}
    </div>
  );
